...
from app.exceptions import EmailAlreadyExistsException, ExpiredVerificationCodeException, InvalidVerificationCodeException, UserNotFoundException, UsernameAlreadyExistsException
...
PENDING_VERIFICATION_MINUTES = 5
...
async def register_email(user_data: SUserRegisterEmail) -> dict[str, str]:
    ...
    existing_user = await UserDAO.find_one_or_none(filters=[User.email == email]) \
        or await redis_client.get(f"user_email_{email}")
    if existing_user:
        raise EmailAlreadyExistsException(email=email)

    hashed_password = get_password_hash(user_data.password)
    code = (
        "".join(random.choices(string.ascii_uppercase + string.digits, k=VERIFICATION_CODE_LENGTH))
    ).upper()
...
@router_auth.post("/register/verify-email")
